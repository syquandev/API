...
public enum ReachabilityXError: Error {
...
public let ReachabilityXChangedNotification = NSNotification.Name("ReachabilityChangedNotification")
...
public class ReachabilityX {
    ...
    public typealias NetworkReachable = (ReachabilityX) -> ()
    public typealias NetworkUnreachable = (ReachabilityX) -> ()
...
public extension ReachabilityX {
    ...
    func startNotifier() throws {
        ...
        let callback: SCNetworkReachabilityCallBack = { (reachability, flags, info) in
            ...
            let reachability = Unmanaged<ReachabilityX>.fromOpaque(info).takeUnretainedValue()
            reachability.flags = flags
        }
        
        var context = SCNetworkReachabilityContext(version: 0, info: nil, retain: nil, release: nil, copyDescription: nil)
        context.info = UnsafeMutableRawPointer(Unmanaged<ReachabilityX>.passUnretained(self).toOpaque())
        if !SCNetworkReachabilitySetCallback(reachabilityRef, callback, &context) {
            stopNotifier()
            throw ReachabilityXError.UnableToSetCallback
        }
        
        if !SCNetworkReachabilitySetDispatchQueue(reachabilityRef, reachabilitySerialQueue) {
            stopNotifier()
            throw ReachabilityXError.UnableToSetDispatchQueue
        }
...
fileprivate extension ReachabilityX {
    ...
    func setReachabilityFlags() throws {
        try reachabilitySerialQueue.sync { [unowned self] in
            ...
            if !SCNetworkReachabilityGetFlags(self.reachabilityRef, &flags) {
                self.stopNotifier()
                throw ReachabilityXError.UnableToGetInitialFlags
            }
...
extension SCNetworkReachabilityFlags {
    ...
    typealias Connection = ReachabilityX.Connection
